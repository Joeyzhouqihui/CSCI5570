...
Gorder::Gorder() : vsize(0), edgenum(0), W(5) {}

void Gorder::readGraph(std::string filename) {
  std::ifstream file(filename.c_str(), std::ios::in);
	...
	while(file>>u>>v){
		...
		if(v>vsize)
			vsize=v;
		edges.push_back(std::make_pair<int, int>(u, v));
		edgenum++;
	}
	vsize++;
	file.close();
	...
	std::sort(edges.begin(), edges.end(), [](const std::pair<int, int>& a, const std::pair<int, int>& b)->bool{
		if(a.first<b.first)
			return true;
	...
	std::vector< pair<int, int> >().swap(edges);
#ifndef Release	
	std::vector<int> inpos(vsize);
...
void Gorder::RCMOrder(std::vector<int>& order) {
	std::queue<int> que;
	...
	std::vector<int> tmp;
	std::vector<int> degreevertex(vsize);
	for(int i=0; i<vsize; i++){
		degreevertex[i]=i;
	}

	std::sort(degreevertex.begin(), degreevertex.end(), [&](const int& a, const int& b)->bool{
...
//				BFSflag[now]=true;
				...
				std::sort(tmp.begin(), tmp.end(), [&](const int& a, const int& b)->bool{
					if(graph[a].outdegree+graph[a].indegree<graph[b].outdegree+graph[b].indegree)
						return true;
					else
						return false;
				...
				if(tmp.size()!=graph[now].outdegree)
					std::cout << "tmp.size()!=graph[now].outdegree" << std::endl;
...
//					if((BFSflag[tmp[i]]==false)&&(QueFlag[tmp[i]]==false)){
	...
	if(order.size()!=vsize){
		std::cout << "order.size()!=vsize" << std::endl;
		quit();
	}
...
Gorder::Transform() {
	...
	if(graph.size()!=(vsize+1)){
		std::cout << "graph.size()!=(vsize+1)" << std::endl;
		quit();
	}

	std::vector<int>().swap(inedge);
	std::vector<std::pair<int, int> > edges;
	edges.reserve(edgenum);
	for(int i=0; i<vsize; i++){
		for(int j=graph[i].outstart, limit=graph[i+1].outstart; j<limit; j++)
			edges.push_back(std::make_pair<int, int>(order[i], order[outedge[j]]));
	}
	if(edges.size()!=edgenum){
		std::cout << "edges.size()!=edgenum" << std::endl;
		quit();
	}
...
void Gorder::GorderGreedy(std::vector<int>& order, int window) {
